...
require("In_Out")
require("PID")
yawPID = CreateNewPID(25, 1, 5, 0.1, 100, 100)
PI2 = math.pi*2
...
function onTick()
	...
	zeTarget:set(1, (vehicleRotation.z*PI2) + math.atan(targetRelativePosition.x, targetRelativePosition.y), (zePitch/4)*PI2)
	zeTarget:toCartesian()
	turretRollVector:set(0, (PI2/4)*turretRoll, 0)
	zeTarget:rotate3D(turretRollVector)
	if enable then
		outputNumbers[14] = (math.asin(zeTarget.z)/PI2)*4
		wrappedYaw = yawSpeed(turretYaw, math.atan(zeTarget.x, zeTarget.y)/PI2)
		outputBools[1] = fire and (math.abs(wrappedYaw) < 0.065)
	else
