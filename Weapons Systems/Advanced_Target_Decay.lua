...
do
    ...
    simulator:setProperty("Tick Correction", 0)
    simulator:setProperty("Acceleration Decay", 1)
    simulator:setProperty("Jerk Decay", 0.1)
    simulator:setProperty("Sample Age", 1)
    ...
    return {
        ...
        positionInTicks = function (self, ticks)
            ...
            for t = 1, ticks, 1 do
                ...
                self.acceleration.predicted:setAdd(self.acceleration.predicted, self.jerk.predicted)
                self.jerk.predicted:setScale(jerkDecay)
                self.acceleration.predicted:setScale(accelerationDecay)
            end
            self.jerk.predicted:setScale(ticks%1)
            self.acceleration.predicted:setAdd(self.acceleration.predicted, self.jerk.predicted)
            self.acceleration.predicted:setScale(ticks%1)
            self.velocity.predicted:setAdd(self.velocity.predicted, self.acceleration.predicted)
            self.velocity.predicted:setScale(ticks%1)
            self.position.predicted:setAdd(self.position.predicted, self.velocity.predicted)
        end
    }
end
...
--[[
testStartPos = newExtendedVector(0, 0, 0)
halfPos = math.cos(math.pi/4)*4
testPoses = {
    newExtendedVector(1, 1, 1),
    newExtendedVector(3, 3, 3),
    newExtendedVector(6, 6, 6)
}
realPoses = {
    newExtendedVector(1, 1, 1),
    newExtendedVector(3, 3, 3),
    newExtendedVector(6, 6, 6),
    newExtendedVector(10, 10, 10),
    newExtendedVector(15, 15, 15)
}
testTarget = newTarget(testStartPos, 1, 1)
for index, testValue in ipairs(testPoses) do
    testTarget:update()
    testTarget:refresh(testValue)
end
...
function onDraw()
    ...
    for i = 0, 5, 1 do
        testTarget:positionInTicks(i)
        screen.drawCircleF(w/2+testTarget.position.predicted.x*zoom, h/2-testTarget.position.predicted.y*zoom, 2)
    end
    screen.setColor(200, 200, 200)
    for index, testValue in ipairs(realPoses) do
        screen.drawCircleF(w/2+testValue.x*zoom, h/2-testValue.y*zoom, 2)
    end
end]]