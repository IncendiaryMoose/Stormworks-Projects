...
function onTick()
    ...
    for i = 1, 17 do
        ...
        if laserDist > 20 and laserDist < colorDepth then
            ...
            local laserZIndex = math.floor((worldPoint.z/worldVerticalRes)+0.5)
            ...
            if not worldPoints[laserZIndex] then
                worldPoints[laserZIndex] = {}
            end
            if not worldPoints[laserZIndex][laserYIndex] then
                worldPoints[laserZIndex][laserYIndex] = {}
            end
            local pointToReplace = worldPoints[laserZIndex][laserYIndex][laserXIndex]
            if (not pointToReplace) or (laserError < pointToReplace.w) then
                worldPoints[laserZIndex][laserYIndex][laserXIndex] = worldPoint
            end
        end
    end
...
function onDraw()
    ...
    local drawBuffer = {}
    for pointZIndex = -renderDist, renderDist do
        pointLayer = screenPoints[pointZIndex]
        if pointLayer then
            for pointYIndex = -renderDist, renderDist do
                pointRow = pointLayer[pointYIndex]
                if pointRow then
                    for pointXIndex = -renderDist, renderDist do
                        pixel = pointRow[pointXIndex]
                        if pixel then
                            if pixel.w < colorDepth then
                                --, math.min(255-pz*255,opacity))
                                --[[for aZ = -1, 1 do
                                    for aY = -1, 1 do
                                        for aX = -1, 1 do
                                            adjacentPixel = screenPoints[pointZIndex+aZ] and screenPoints[pointZIndex+aZ][pointYIndex+aY] and screenPoints[pointZIndex+aZ][pointYIndex+aY][pointXIndex+aX]
                                        end
                                    end
                                end]]
                                pixelLeft = pointRow[pointXIndex-1]
                                pixelRight = pointRow[pointXIndex+1]
                                pixelBehind = (pointLayer[pointYIndex+1] and pointLayer[pointYIndex+1][pointXIndex])
                                pixelFront = (pointLayer[pointYIndex-1] and pointLayer[pointYIndex-1][pointXIndex])
                                pixelBelow = (screenPoints[pointZIndex-1] and screenPoints[pointZIndex-1][pointYIndex] and screenPoints[pointZIndex-1][pointYIndex][pointXIndex])
                                pixelAbove = (screenPoints[pointZIndex+1] and screenPoints[pointZIndex+1][pointYIndex] and screenPoints[pointZIndex+1][pointYIndex][pointXIndex])
                                if pixelBehind then
                                    if pixelRight then
                                        table.insert(drawBuffer, {depth = pixel.w, p1 = pixel, p2 = pixelRight, p3 = pixelBehind})
                                    end
                                    if pixelAbove then
                                        table.insert(drawBuffer, {depth = pixel.w, p1 = pixel, p2 = pixelAbove, p3 = pixelBehind})
                                    end
                                end
                                if pixelFront then
                                    if pixelLeft then
                                    ...
                                    if pixelBelow then
                                        table.insert(drawBuffer, {depth = pixel.w, p1 = pixel, p2 = pixelBelow, p3 = pixelFront})
                                    end
                                end
                                if pixelLeft then
                                    if pixelBelow then
                                        table.insert(drawBuffer, {depth = pixel.w, p1 = pixel, p2 = pixelLeft, p3 = pixelBelow})
                                    end
                                end
                                if pixelRight then
                                    if pixelAbove then
                                        table.insert(drawBuffer, {depth = pixel.w, p1 = pixel, p2 = pixelRight, p3 = pixelAbove})
                                    end
                                end
                            end
                        end
    ...
    for i, tri in ipairs(drawBuffer) do
        ...
        screen.setColor(r, g, b, math.min(255-pz*255,opacity))
        drawTri(tri)
    end
end
...
worldToScreenPoint = function(points)
    screenPoints = {}
    for pointZIndex = -renderDist, renderDist do
        pointLayer = points[vehiclePositionIndex.z + pointZIndex]
        if pointLayer then
            for pointYIndex = -renderDist, renderDist do
                pointRow = pointLayer[vehiclePositionIndex.y + pointYIndex]
                if pointRow then
                    for pointXIndex = -renderDist, renderDist do
                        ...
                        if point then
                            ...
                            if sz > 0 and sz < colorDepth and sx >= -fov and sx <= w + fov and sy >= -fov and sy <= h + fov then
                                if not screenPoints[pointZIndex] then
                                    screenPoints[pointZIndex] = {}
                                end
                                if not screenPoints[pointZIndex][pointYIndex] then
                                    screenPoints[pointZIndex][pointYIndex] = {}
                                end
                                screenPoints[pointZIndex][pointYIndex][pointXIndex] = {
                                    x = sx,
                                    y = sy,
                                    z = sz,
