...
function onTick()
    ...
    if EngineReadyTimer == 60 then
        ...
        Lock = ((not Combat) and (velocity.y < lockSpeed))
        outputRotor(1, altitude_pids[1].output, yHoldPID.output, xHoldPID.output + headingPID.output, 0.3, 0.3, 0.4, 0.65)
        outputRotor(6, altitude_pids[2].output, yHoldPID.output, -xHoldPID.output - headingPID.output, 0.3, 0.3, 0.4, 0.65)
        outputRotor(11, altitude_pids[3].output, yHoldPID.output, xHoldPID.output - headingPID.output, 1, 0.2, 0.2, 1)
        outputRotor(16, altitude_pids[4].output, yHoldPID.output, -xHoldPID.output + headingPID.output, 1, 0.2, 0.2, 1)
        outputNumbers[21] = altitude_pids[1].output*sign(velocity.y)
        outputNumbers[22] = altitude_pids[2].output*sign(velocity.y)
        outputNumbers[23] = altitude_pids[3].output*sign(velocity.y)
        outputNumbers[24] = altitude_pids[4].output*sign(velocity.y)
        outputNumbers[25] = yHoldPID.output
        outputNumbers[26] = xHoldPID.output
        outputNumbers[27] = headingPID.output*sign(velocity.y)/2
        outputBools[4] = Lock
        outputBools[5] = auto
...
function outputRotor(startChannel, collective, p, r, pMax, rMax, r2Max, p2Max)
    outputNumbers[startChannel] = clamp(-collective, -1, 0)
    if Lock then
        outputNumbers[startChannel + 1] = clamp(p, -pMax, pMax)
        outputNumbers[startChannel + 2] = clamp(r, -rMax, rMax)
        outputNumbers[startChannel + 3] = clamp(r, -r2Max, r2Max)
        outputNumbers[startChannel + 4] = clamp(p, -p2Max, p2Max)
    else
        outputNumbers[startChannel + 1] = clamp(p, -1, 1)
        outputNumbers[startChannel + 2] = clamp(r, -1, 1)
    end
end
